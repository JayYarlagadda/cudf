...
@pytest.mark.parametrize("num_files", [1, 3])
def test_columnchunk_metadata_standalone_from_footers(
    tmp_path: Path, num_files: int
) -> None:
    """Regression for standalone columnchunk_metadata(footers) with multiple sources."""
    table = pa.table(
        {
            "a": list(range(10)),
            "b": [x * 10 for x in range(10)],
    ...
    for i in range(num_files):
        parquet_path = (
            tmp_path / f"standalone-columnchunk-metadata-{i}.parquet"
        )
        write_table(table, parquet_path, row_group_size=5)
        parquet_paths.append(parquet_path)
    ...
    got = plc.io.parquet_metadata.columnchunk_metadata(footers)
